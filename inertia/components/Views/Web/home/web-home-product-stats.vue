...
<template>
  <div :style="{ 'background-color': 'rgb(var(--v-theme-surface))' }">
    <VContainer>
      <div class="py-12">
        <VRow>
          <VCol v-for="(product, index) in statData" :key="index" cols="12" sm="6" lg="3">
            <VCard flat :style="{ border: `1px solid rgba(var(--v-theme-${product.color}))` }">
              <VCardText class="text-center">
                <VIcon :color="product.color" :icon="product.icon" size="64" class="mb-4" />
                <h3 class="text-h3">
                  {{ product.value }}
                </h3>

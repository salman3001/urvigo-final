...
<template>
  <VRow>
    <VCol v-for="article in props.categories" :key="article.title" cols="12" sm="6" lg="4">
      <VCard :title="article.title">
        <template #prepend>
          <VAvatar rounded color="primary" variant="tonal" size="32" class="me-1">
            <VIcon :icon="article.icon" size="20" />
          </VAvatar>
        </template>

        <VCardText>
          <VList class="card-list">
            <VListItem v-for="(item, index) in article.articles" :key="index" class="text-disabled">
              <Link :href="''" class="text-high-emphasis">
                <div>{{ item.title }}</div>
              </Link>
              <template #append>

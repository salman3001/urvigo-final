  ...
  <ModalBase v-model:is-visible="model" title="" subtitle="">
    <VCardItem v-if="selectedBid">
      <div class="d-flex gap-2 flex-wrap justify-space-between">
        <div class="d-flex gap-2">
          <VAvatar :image="getImageUrl(selectedBid?.vendor?.profile?.avatar?.url, dummyAvatar)" size="x-large" />
          <div>
            <div>
              <RatingComponent :rating="selectedBid?.vendor?.businessProfile.avgRating || 0" size="large" />
            </div>
            <div class="d-flex gap-2" v-if="acceptedBid?.id === selectedBid?.id">
              ...
              <VBtn size="small" v-if="acceptedBid?.id === selectedBid?.id" @click="() => emit('createChat')">chat
              </VBtn>
            </div>
            <p v-else>Anonymous</p>
            <div>
              <Link :href="routes('web.vendor-profile.about', [selectedBid.vendor.id])">{{
    selectedBid?.vendor?.businessProfile?.businessName }}</Link>
            </div>
          </div>
        </div>
        <div>
      ...
      <div>
        Price Offered
        <span class="font-weight-bold">&#x20B9;{{ new BigNumber(selectedBid?.offeredPrice || 0).toFixed(2) }}</span>
      </div>
      <br />
      <div>
        <p>
      ...
      <div v-if="selectedBid?.negotiateHistory.length">
        ...
        <br />
        <VTimeline truncate-line="both" line-inset="9" align="start" side="end" line-color="primary" density="compact">
          <VTimelineItem v-for="(h, i) in selectedBid?.negotiateHistory" :key="i" dot-color="primary" size="x-small">
    ...
    <VCardItem v-if="selectedBid && !acceptedBid">
      <div class="d-flex justify-end gap-2">
        <VBtn variant="tonal" color="primary" @click.prevent="negotiateModal = true" v-if="!selectedBid?.negotiateHistory?.length ||
    selectedBid?.negotiateHistory[selectedBid?.negotiateHistory?.length - 1]?.accepted
    ">Negotiate</VBtn>
        <Link :href="routes('web.custom_booking.summary') +
    ...
    <ModalBidNegotiate v-model="negotiateModal" :selected-bid="selectedBid" :service-requirement="serviceRequirement"
      @negotiated="() => {

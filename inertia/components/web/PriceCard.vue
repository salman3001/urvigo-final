...
watch(
  ...
  () => {
    ...
    if (props.selectedVariant?.discount_type === DiscountType.FLAT) {
      discount.value = discount.value.plus(props.selectedVariant?.discount_flat || 0)
    } else if (props.selectedVariant?.discount_type === DiscountType.PERCENATAGE) {
      discount.value = new BigNumber(props.selectedVariant?.discount_percentage || 0)
        .div(100)
  ...
  <div>
    <div v-if="selectedVariant" class="column q-col-gutter-lg">
      ...
      <div class="text-h5">
        <NuxtLink
          :to="{
            path: routes('web.booking.summary', [selectedVariant.id]),
            query: { qty: qty },
          }"
        >

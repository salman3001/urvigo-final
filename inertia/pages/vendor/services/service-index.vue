    ...
    <VContainer>
                ...
                <VCardText>
                    <div class="d-flex justify-sm-space-between justify-start flex-wrap gap-4">
                        ...
                        <div class="d-flex gap-x-4 align-center">
                            <AppSelect v-model="query.serviceCategoryId"
                                :items="[...categories, { name: 'None', id: '' }]" item-title="name" item-value="id"
                                label="Categories" style="min-inline-size: 260px" />
                            <AppSelect v-model="query.serviceSubcategoryId"
                                :items="[...subcategories, { name: 'None', id: '' }]" item-title="name" item-value="id"
                                label="Sub Categories" style="min-inline-size: 260px" />
            ...
            /> -->
                ...
                <VDataTableServer v-model:items-per-page="query.perPage!" v-model:page="query.page" :headers="headers"
                    ...
                    <template #item.tags="{ item }">
                        <VChip v-for="tag in item?.tags" label size="small" :text="tag.name" color="info"
                            :key="tag.id" />
                    </template>
        ...
        <ModalConfirm v-model:is-visible="confirmModal" title="Update Status"
            message="Change status of the service are your sure? " @confirmed="() => {
                            updateStatusForm.post(routes('vendor.service.edit.post', [selectedServiceId || 0]), {
                                onSuccess: () => {
                                    router.reload({
                                        only: ['services'],
                                        data: query,

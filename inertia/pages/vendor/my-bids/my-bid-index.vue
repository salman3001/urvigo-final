...
import type Bid from '../../../../app/models/bid';
...
watch(selectedBid, () => {
    if (selectedBid.value) acceptNegotiaionForm.newPrice = selectedBid.value?.negotiateHistory[selectedBid.value?.negotiateHistory?.length - 1]?.asked_price || ''
})
...
<template>
            ...
            <VCard>
                <VCardItem>
                    <div class="d-flex gap-2 flex-wrap justify-space-between">
                        ...
                        <div>
                            <VChip v-if="bid.id === bid?.serviceRequirement?.acceptedBidId" color="success"> Accepted
                            </VChip>
                        </div>
                    </div>
                    <br />
                ...
                <VCardActions class="justify-end mt-2">
                    <VBtn color="primary" @click="() => { selectedBid = bid; detailModal = true; }">Review</VBtn>
                </VCardActions>
                <br />
            </VCard>
        </VCol>
    ...
    <br>
    <TablePagination :page="Number(query.page)" :items-per-page="Number(bids?.meta?.perPage)"
        :total-items="Number(bids?.meta?.total)" @update:page="(p) => {
            query.page = p
        }
            " />
        ...
        <VCardItem v-if="selectedBid">
            <div class="d-flex gap-2 flex-wrap justify-space-between">
                <div>
                    ...
                    <VChip v-if="selectedBid.id === selectedBid?.serviceRequirement?.acceptedBidId" outline
                        color="success">
                        Accepted</VChip>
                </div>
            </div>
            ...
            <div>
                Price Offered
                <span class="font-weight-bold">&#x20B9;{{ new BigNumber(selectedBid?.offeredPrice || 0).toFixed(2)
                    }}</span>
            </div>
            <br />
            <div>
            ...
            <div v-if="selectedBid && selectedBid.negotiateHistory?.length > 0">
                ...
                <div>
                    <VTimeline truncate-line="both" line-inset="9" align="start" side="end" line-color="primary"
                        density="compact">
                        <VTimelineItem v-for="(h, i) in selectedBid?.negotiateHistory" :key="i" dot-color="primary"
                            size="x-small">

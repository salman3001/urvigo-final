...
const _syncAppRtl = () => {
  ...
  if (locale.value !== storedLang.value && storedLang.value) locale.value = storedLang.value
    ...
    (val) => {
      ...
      if (themeConfig.app.i18n.langConfig && themeConfig.app.i18n.langConfig.length) {
        themeConfig.app.i18n.langConfig.forEach((lang) => {
          if (lang.i18nLang === storedLang.value) configStore.isAppRTL = lang.isRTL
        })
      }
    },
    { immediate: true }
...
const _syncInitialLoaderTheme = () => {
  ...
  watch(
    ...
    () => {
      ...
      useStorage<string | null>(namespaceConfig('initial-loader-bg'), null).value =

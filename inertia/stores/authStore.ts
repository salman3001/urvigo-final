...
import { ref } from 'vue'
import type { userTypes } from '@/utils/enums'

const authStore = defineStore('Auth', () => {
  const { fetcher, fetchRef, errors } = useFetchRef()
  const login = async (email: string, password: string, userType: userTypes) => {
    try {
  ...
  const verifyOtpAndUpdatePWD = (cb?: { onSuccess?: () => void; onError?: () => void }) => {
    const loading = ref(false)
